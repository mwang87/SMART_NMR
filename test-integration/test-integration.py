...
def test_api():
    ...
    for test_file in test_files:
        ...
        r = requests.post(f"{PRODUCTION_URL}/api/classic/search", data={"peaks":json.dumps(peaks_json)})
        r.raise_for_status()

        r = requests.post(f"{PRODUCTION_URL}/api/classic/search", data={"peaks":json.dumps(peaks_json), "json":"json"})
        print(r.json())
        r.raise_for_status()
